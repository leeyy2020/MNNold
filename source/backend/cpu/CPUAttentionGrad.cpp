...
ErrorCode CPUAttentionGrad::onExecute(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    if (!sPrintedOnce) {
        MNN_PRINT("[CPUAttentionGrad::onExecute] inputs summary:\n");
        printF32Summary(query, "query");
        printF32Summary(key,   "key");
        printF32Summary(value, "value");
        printF32Summary(output_grad, "output_grad");
        if (mask) {
            MNN_PRINT("  mask: shape=");
            printTensorShape(mask);
            MNN_PRINT(", dtype=%d\n", mask->getType().code);
        } else {
            MNN_PRINT("  mask: <null>\n");
        }
        if (sinks) {
            MNN_PRINT("  sinks: shape=");
            printTensorShape(sinks);
            MNN_PRINT(", dtype=%d\n", sinks->getType().code);
        } else {
            MNN_PRINT("  sinks: <null>\n");
        }
    }
    ...
    if (!sPrintedOnce) {
        MNN_PRINT("[CPUAttentionGrad::onExecute] outputs summary:\n");
        printF32Summary(query_grad, "query_grad");
        printF32Summary(key_grad,   "key_grad");
        printF32Summary(value_grad, "value_grad");
        sPrintedOnce = true;
    }
